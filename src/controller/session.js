...
exports.login = (req, res) => {
    ...
    db.query("SELECT * FROM user where id = ? and password = ?;", [username, hash(password)], (err, result) => {
        if (err) res.status(500).json({message : "서버 오류, 잠시 후 다시 시도해주세요 !"})
        if (result.length > 0){
            req.session.user = {username};
            res.status(200).json({message : "로그인 성공"})
        }
        else res.status(400).json({message : "로그인 실패, 아이디 또는 비밀번호를 확인해주세요 !"})
    })
}
...
exports.register = (req, res) => {
    ...
    db.query("SELECT * FROM user where id = ?;", username, (err, result) => {
        if (err) res.status(500).json({message : "서버 오류, 잠시 후 다시 시도해주세요!"})
        if (result.length > 0) res.status(201).json({message : "이미 존재하는 ID 입니다."})
        else{
            db.query("INSERT INTO user (id, password) VALUES (?, ?)", [username, hash(password)], (e, result) => {
                if (e) res.status(500).json({message : "서버 오류, 잠시 후 다시 시도해주세요!"})
